...
\begin{frame}{Explicit evaluations}
    ...
    \only<2>{
    \begin{theorem}\label{thm:symcase}For \gls{aa} applied to a symmetric matrix
    ...
    \begin{theorem}\label{thm:polyskewsym}
        For \gls{aa} applied to a skew-symmetric $\gls{A}$ with $\|\gls{A}\|\leq
        \gls{emax}$ we have
        \begin{equation}
...
\begin{frame}{Evaluation of Polynomial style bounds}
    ...
    \only<2>{
        \begin{figure}
            \begin{center}
                \input{images/plots/symmetricresidual.tikz}  
            \end{center}
...
\begin{frame}{Approach II: Some geometric intuition (2D)}
\begin{figure}
    \centering
    \input{tikz/circle}
