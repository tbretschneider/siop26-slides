...
\begin{frame}[allowframebreaks]{Behaviour of $\gls{rho}[i]$ in higher dimensions}
    ...
    \end{equation}
    where the middle matrix is formed of the cosines of the inner products between the
    vectors, and has the following structure,
    \begin{equation}
        \gls{KMSmat}[k] :=
        \begin{pmatrix}
    ...
    this is something like an inhomogeneous \gls{kms} \parencite{Kac1953}, (or the
...
\begin{frame}{Perturbations of $\gls{KMSmat}[k]$}
    ...
    \begin{theorem}\label{thm:perturbation}
        ...
        \begin{equation}
            ...
            +
            2(\gls{KMSl}\cdot\gls{e}_k)\left(\frac{1-\gls{KMSlpert}\cdot\gls{KMSl}}{\gls{KMSlpert}\cdot\gls{e}[k]}\right)\geq
            1
        \end{equation}
    \end{theorem}
